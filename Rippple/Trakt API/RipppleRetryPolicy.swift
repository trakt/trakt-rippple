//
//  RipppleRetryPolicy.swift
//  Rippple
//
//  Created by Kevin Cador on 28/10/2020.
...
final class RipppleRetryPolicy: RequestInterceptor {
    ...
    func retry(_ request: Request,
        ...
        if let statusCode = request.response?.statusCode,
           statusCode == 429 {
            // Rate Limit Exceeded
            if let retry = request.response?.allHeaderFields["retry-after"] as? String {
                completion(.retryWithDelay(TimeInterval(Int(retry)!)))
            } else if request.retryCount < retryLimit {
                completion(.retryWithDelay(pow(Double(exponentialBackoffBase), Double(request.retryCount)) * exponentialBackoffScale))
            } else {
                completion(.retryWithDelay(TimeInterval(60)))
            }
            return
        }
        ...
        if let statusCode = request.response?.statusCode,
           ...
           statusCode == 423 {
            DispatchQueue.main.async {
                UIApplication.shared.switchToLogin423()
            }
        ...
        if let statusCode = request.response?.statusCode,
           ...
           statusCode == 426, request.request?.method != .get {
            DispatchQueue.main.async {
                UIApplication.shared.vipOnly()
            }
            completion(.doNotRetry)
            return
        }
