...
final class FriendsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if user.isCurrentUser {
            ...
            navigationItem.title = "\(user.username)'s Friends"
        }
    ...
    func fetch(pageInfo: PageInfo) {
        print("Fetching page \(pageInfo.page) for friends")

        TraktAPIProvider.provider.request(.friends(slug: user.slug), callbackQueue: DispatchQueue.global(qos: .userInitiated)) { [weak self] result in
            ...
            case .success(let moyaResponse):
                do {
                    ...
                    snapshot.appendItems(self.friends.map { Wrapper.friends($0) })
...
extension FriendsViewController {
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard case Wrapper.friends(let user) = item else { return }
