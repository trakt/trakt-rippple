...
final class RecommendedViewController: UITableViewController {
    ...
    private var error: Error? {
        didSet {
            if let error = error {
                errorLabel.text = "An error occurred while fetching your personal recommendations.\n\(error.localizedDescription)"
            } else {
                errorLabel.text = "An error occurred while fetching your personal recommendations..."
            }
        }
    ...
    private var currentFilter = Filter.none {
        didSet {
            if user.isCurrentUser {
                UserDefaults.standard.set(currentFilter.rawValue, forKey: "RecommendedViewController.currentFilter")
    ...
    func cycleFilter() {
        let cycle: [Filter] = [.movies, .shows, .none]
    ...
    private var currentSorting = Sort.rank {
        didSet {
            UserDefaults.standard.set(currentSorting.rawValue, forKey: "RecommendedViewController.currentSorting")
            UserDefaults.standard.synchronize()
    ...
    override func viewDidLoad() {
        ...
        if user.isCurrentUser {
            if let filter = Filter(rawValue: UserDefaults.standard.integer(forKey: "RecommendedViewController.currentFilter")) {
                currentFilter = filter
            }

            if let sort = Sort(rawValue: UserDefaults.standard.integer(forKey: "RecommendedViewController.currentSorting")) {
                currentSorting = sort
            }
        ...
        onWatchlistTypeNotesChangedReceiver.listen { [weak self] watchlistType in
            guard let self = self else { return }
            if case .favoriteItem = watchlistType {
                self.fetch()
            }
        }.disposed(by: disposeBag)
    ...
    func fetch() {
        ...
        guard case .recommended(let slug, let type, let extended, let sort, _) = service else { return }
    ...
    private func filterMenu() -> UIMenu {
        let deferredMenuElement = UIDeferredMenuElement.uncached { completion in
            ...
            let filters = UIMenu(title: "What do you want to see?", options: .displayInline, children: [all, movies, shows])
...
extension RecommendedViewController {
    ...
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let remove = UIContextualAction(style: .normal,
            ...
            media.removeFromRecommendations()
            boolValue(true)
        }
    ...
    private func displayNotes(for watchlistItem: WatchlistItem) {
        NotesManager.shared.showNotes(for: WatchlistType.favoriteItem(note: watchlistItem.notes ?? "",
