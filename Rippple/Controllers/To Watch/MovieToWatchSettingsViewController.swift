//
//  MovieToWatchSettingsViewController.swift
//  Rippple
//
...
struct MovieToWatchListItem: Codable, Identifiable, Hashable {
...
final class MovieToWatchSettingsViewModel: ObservableObject {
    ...
    @Published var otherListItems: [MovieToWatchListItem]
    ...
    private let settings = MovieToWatchSettings.shared
    ...
    init() {
        ...
        availableSmartSearches = SmartSearch.smartSearches(for: .movie)
    ...
    func setOtherList(_ item: MovieToWatchListItem, enabled: Bool) {
    ...
    func isAdded(_ list: List, kind: MovieToWatchListItem.Kind) -> Bool {
        settings.otherLists.contains { $0.matches(kind: kind, list: list) }
    }

    func select(sort newSort: MovieToWatchSettings.Sort) {
        applyChange {
            if sort != newSort {
                reverse = false
    ...
    func setUpcomingEnabled(_ newValue: Bool) {
        ...
        UserDefaults.standard.setValue(newValue, forKey: "MovieToWatchSettings.upcoming")
        UserDefaults.standard.synchronize()
        movieUpcomingEnabledTransmitter.broadcast(newValue)
    }

    func setGroupMode(_ newValue: MovieToWatchGroupMode) {
        guard PurchaseManager.shared.purchased else {
            UIApplication.shared.switchToPurchase()
            return
        }
        groupMode = newValue
        newValue.persist()
        movieToWatchGroupModeTransmitter.broadcast(newValue)
    }

    func label(for sort: MovieToWatchSettings.Sort) -> String {
        ...
        case .runtime:
            return reversed ? "Shortest" : "Longest"
        case .random:
            return reversed ? "Still Random" : "Random"
        case .automatic:
            return reversed ? "Automatic (Reversed)" : "Automatic"
        case .title:
    ...
    func broadcastIfNeeded() {
        ...
        shouldRefresh = false
        movieToWatchSettingsUpdatedTransmitter.broadcast(settings)
    }

    private func addOtherList(kind: MovieToWatchListItem.Kind,
                              smartSearch: SmartSearch?,
                              list: List?) {
        updateOtherLists { configs in
            let identifier = MovieToWatchListItem.identifier(kind: kind,
                                                             smartSearch: smartSearch,
                                                             list: list)
    ...
    private func updateOtherLists(_ transform: (inout [MovieToWatchListItem]) -> Void) {
    ...
    private func nextRank(from configs: [MovieToWatchListItem]? = nil) -> Int {
    ...
    private func replaceOtherLists(with items: [MovieToWatchListItem]) {
    ...
    private func updateSetting(_ stateKeyPath: ReferenceWritableKeyPath<MovieToWatchSettingsViewModel, Bool>,
                               settingsKeyPath: ReferenceWritableKeyPath<MovieToWatchSettings, Bool>,
                               to newValue: Bool) {
        applyChange {
    ...
    private func bindReceivers() {
        ...
        onMovieSmartSearchChangedReceiver.hotOnly().listen { [weak self] _ in
            guard let self else { return }
            availableSmartSearches = SmartSearch.smartSearches(for: .movie)
            pruneUnavailableOtherLists()
        }.disposed(by: disposeBag)
...
struct MovieToWatchSettingsView: View {
    ...
    private func otherListRow(item: MovieToWatchListItem) -> some View {
        HStack(spacing: 12) {
...
struct MovieAddListPickerView: View {
    ...
    init(viewModel: MovieToWatchSettingsViewModel) {
        ...
        if filteredSmartSearches.isEmpty == false {
            Section("Smart Lists") {
                ForEach(filteredSmartSearches, id: \.self) { smartSearch in
                    let id = MovieToWatchListItem.identifier(kind: .smartSearch,
                                                             smartSearch: smartSearch,
                                                             list: nil)
                    selectionRow(id: id,
    ...
    private func addItem(with id: String) {
        if let smartSearch = viewModel.availableSmartSearches.first(where: { MovieToWatchListItem.identifier(kind: .smartSearch, smartSearch: $0, list: nil) == id }) {
            viewModel.addSmartSearch(smartSearch)
            return
        }
        if let list = viewModel.availableCustomLists.first(where: { MovieToWatchListItem.identifier(kind: .customList, smartSearch: nil, list: $0) == id }) {
            viewModel.addCustomList(list)
            return
        }
        if let list = viewModel.availableLikedLists.first(where: { MovieToWatchListItem.identifier(kind: .likedList, smartSearch: nil, list: $0) == id }) {
            viewModel.addLikedList(list)
            return
        }
        if let list = viewModel.availableCollaborations.first(where: { MovieToWatchListItem.identifier(kind: .collaboration, smartSearch: nil, list: $0) == id }) {
...
final class MovieToWatchSettingsViewController: UIHostingController<MovieToWatchSettingsView> {
    ...
    required init?(coder aDecoder: NSCoder) {
        super.init(coder: aDecoder, rootView: MovieToWatchSettingsView(viewModel: viewModel))
    }
...
final class MovieToWatchSettings {
    ...
    private init() {
        watchlist = UserDefaults.standard.bool(forKey: "MovieToWatchSettings.watchlist")
        recommended = UserDefaults.standard.bool(forKey: "MovieToWatchSettings.recommended")
        collected = UserDefaults.standard.bool(forKey: "MovieToWatchSettings.collected")
        ...
        onMovieSmartSearchChangedReceiver.hotOnly().listen { [weak self] _ in
            guard let self else { return }
            let smartSearchMovies = SmartSearch.smartSearches(for: .movie)
            refreshOtherLists(smartSearches: smartSearchMovies)
        }.disposed(by: disposeBag)
    }
    ...
    static let shared = MovieToWatchSettings()

    var otherLists = [MovieToWatchListItem]() {
        didSet {
            if let encoded = try? JSONEncoder().encode(otherLists) {
                UserDefaults.standard.set(encoded, forKey: "MovieToWatchSettings.otherLists")
                UserDefaults.standard.synchronize()
            }
            setDefaultIfNeeded()
        }
    ...
    var watchlist = true {
        didSet {
            UserDefaults.standard.set(watchlist, forKey: "MovieToWatchSettings.watchlist")
            UserDefaults.standard.synchronize()
        }
    }
    ...
    private func ensureSequentialRanks() {
        let normalized = otherLists.enumerated().map { index, item -> MovieToWatchListItem in
            var updated = item
            updated.rank = index
            return updated
        }
    ...
    private func refreshOtherLists(customLists: [List]? = nil,
        ...
        otherLists = updated
        movieToWatchSettingsUpdatedTransmitter.broadcast(self)
    }
}

extension MovieToWatchSettingsViewController: UIAdaptivePresentationControllerDelegate {
