//
//  DroppedShowsViewController.swift
//  Rippple
//
...
class DroppedShowsViewController: UITableViewController {
    ...
    private lazy var dataSource = DroppedDiffibleDataSource(tableView: tableView) { [weak self] tableView, _, item in
        ...
        switch item {
        case .infoCard:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "info card") as? InfoCardTableViewCell else {
                fatalError("Could not dequeue an info card cell")
            }

            cell.infoImage.isHidden = true
            cell.title.text = "About Dropped Shows"

            let customStyle = UIImage.SymbolConfiguration(font: UIFont.preferredFont(forTextStyle: .footnote))

            let explicitDropTextAttachment = NSTextAttachment()
            explicitDropTextAttachment.image = UIImage(systemName: "minus.circle")?.withConfiguration(customStyle).withTintColor(.label)

            let automaticallyDropTextAttachment = NSTextAttachment()
            automaticallyDropTextAttachment.image = UIImage(systemName: "minus.arrow.trianglehead.counterclockwise")?.withConfiguration(customStyle).withTintColor(.label)

            let fullString = NSMutableAttributedString(string: "A TV show is ")
            fullString.append(NSAttributedString(attachment: automaticallyDropTextAttachment))
            fullString.append(NSAttributedString(string: " Automatically Dropped, in Rippple only, when you you stop Watching it for more than 6 months.\nA TV show is "))
            fullString.append(NSAttributedString(attachment: explicitDropTextAttachment))
            fullString.append(NSAttributedString(string: " Explicitly Dropped, and synced with Trakt, when you manually 'Drop' it.\nTo Undo a Drop, you just start watching any episode of the TV show again."))
            cell.body.attributedText = fullString
            cell.body.font = UIFont.preferredFont(forTextStyle: .footnote)
    ...
    override func viewDidLoad() {
        ...
        tableView.register(UINib(nibName: "InfoCardTableViewCell", bundle: nil), forCellReuseIdentifier: "info card")
        tableView.dataSource = dataSource
        tableView.separatorStyle = .none

        onDroppedShowsChangedReceiver.listen { [weak self] droppedShows in
            guard let self = self else { return }
            self.refresh(with: droppedShows)
    ...
    func refresh(with hiddenMediaList: [MediaModel]) {
        if hiddenMediaList.isEmpty {
            ...
            snapshot.appendItems([.empty("👊",
                                         "No Dropped Show",
                                         "There are currently no dropped shows in your To Watch",
                                         "")])
...
extension DroppedShowsViewController: MediaTableViewCellDelegate {
